...
exports.PostCategoryGetController = async (req, res, next) => {
    const errors = validationResult(req).formatWith(errorFormatter)
...
exports.DelCategoryGetController = async (req, res, next) => {
    const errors = validationResult(req).formatWith(errorFormatter)
    const { category } = req.body
...
exports.getCategoryGetController = async (req, res, next) => {
    const profile = await Profile.findOne({ user: req.user._id })
    !profile && res.redirect('/dashboard/create-profile')
    try {
        res.render('pages/dashboard/add-category', {
